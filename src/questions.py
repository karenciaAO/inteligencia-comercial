...
def answer(question, sales, actions):
    ...
    empty = pd.DataFrame()
    # Filtrado se controla en la barra lateral: no fingir que interpretamos fechas arbitrarias.
    ...
    if 'campan' in q:
        return Answer('Contexto de campañas',summary(sales,['Contexto_Campania']), 'Coincidencia de fecha y canal; no mide impacto incremental.')
    if 'descuento' in q:
        return Answer('Descuento y rentabilidad',discounts(sales),'Asociación descriptiva; la mezcla de productos puede explicar diferencias.')
    ...
    if 'mensual' in q or 'tendencia' in q:
        return Answer('Evolución mensual',monthly(sales),'Meses sin registros se mantienen sin dato, no como cero.',metric)
    ...
    if 'resumen' in q or 'kpi' in q:
        return Answer('Resumen del negocio',summary(sales),'Margen porcentual ponderado y valor por transacción, no ticket por pedido.')
